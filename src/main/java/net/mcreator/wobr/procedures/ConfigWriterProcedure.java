	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		if (dependencies.get("world") == null) {
			if (!dependencies.containsKey("world"))
				WobrMod.LOGGER.warn("Failed to load dependency world for procedure ConfigWriter!");
			return;
		}
		...
		}
		{
			try {
				...
				WobrModVariables.MapVariables.get(world).KF_Av_Villager = (boolean) config_exist.get("zombie_villager_killed").getAsBoolean();
				WobrModVariables.MapVariables.get(world).syncData(world);
				WobrModVariables.MapVariables.get(world).KF_Av_Pigman = (boolean) config_exist.get("zombie_pigman_killed").getAsBoolean();
				WobrModVariables.MapVariables.get(world).syncData(world);
			} catch (IOException e) {
				e.printStackTrace();
			}
