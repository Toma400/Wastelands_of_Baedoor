...
public class AirshipJungleStructure extends WobrModElements.ModElement {
	private static final Feature<NoFeatureConfig> feature = new Feature<NoFeatureConfig>(NoFeatureConfig::deserialize) {
		...
		public boolean place(IWorld world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
			...
			if (!dimensionCriteria)
				return false;
			if ((random.nextInt(1000000) + 1) <= Config.AIRSHIP_JUNGLE_CHANCE.get()) { // config
	...
	public AirshipJungleStructure(WobrModElements instance) {
		super(instance, 1598);
	...
	public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
		event.getRegistry().register(feature.setRegistryName("airship_jungle"));
	}
