...
public class AirshipMageStructure extends WobrModElements.ModElement {
		...
		public boolean place(IWorld world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
			...
			if ((random.nextInt(1000000) + 1) <= 170) {
				...
				for (int a = 0; a < count; a++) {
					...
					int y = spawnTo.getY();
					int z = spawnTo.getZ();
					if (!AirshipMageConditionProcedure.executeProcedure(ImmutableMap.of("world", world)))
						continue;
					Template template = ((ServerWorld) world.getWorld()).getSaveHandler().getStructureTemplateManager()
							.getTemplateDefaulted(new ResourceLocation("wobr", "spawn_entity_airship_mage"));
					if (template == null)
						return false;
	...
	public AirshipMageStructure(WobrModElements instance) {
		super(instance, 1594);
	...
	public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
		event.getRegistry().register(feature.setRegistryName("airship_mage"));
	}
