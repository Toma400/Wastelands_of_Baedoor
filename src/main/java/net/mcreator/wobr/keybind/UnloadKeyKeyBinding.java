...
import net.mcreator.wobr.procedures.UnloadScoreCheckProcedure;
...
public class UnloadKeyKeyBinding extends WobrModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	...
	public void initElements() {
		keys = new KeyBinding("key.mcreator.unload_key", GLFW.GLFW_KEY_U, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					WobrMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				UnloadScoreCheckProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
