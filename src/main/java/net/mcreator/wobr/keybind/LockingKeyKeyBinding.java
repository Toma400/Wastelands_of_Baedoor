...
import net.mcreator.wobr.procedures.LockingCheckProcedure;
...
public class LockingKeyKeyBinding extends WobrModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	...
	public void initElements() {
		keys = new KeyBinding("key.wobr.locking_key", GLFW.GLFW_KEY_L, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					...
					WobrMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(1, dt));
		...
		if (type == 1) {
			{
				...
				LockingCheckProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
